...
def evaluate(data: dict[str, Any]) -> dict[str, Any]:
    ...
    for case in cases:
        ...
        predicted = {finding.rule_id for finding in run_detections(events, config) if finding.rule_id in rule_universe}
    ...
    return {
        "evaluation_type": "synthetic rule-level case evaluation",
        "case_count": len(cases), "rule_count": len(rule_universe),
        ...
        "false_positive_rate": round(false_positive_rate, 4), "accuracy": round(accuracy, 4),
        "environment": {"python": sys.version.split()[0], "platform": platform.platform()},
        "limitations": [
            "The evaluation cases are synthetic and do not estimate real-world SOC performance.",
            "Metrics are computed over rule/case pairs, not individual production alerts.",
            "Results should be replicated on public or independently curated datasets before broader claims are made."
        ],
        "cases": details,
    }
...
def main() -> int:
    ...
    parser.add_argument("--dataset", type=Path, default=Path("evaluation/labelled_cases.json"), help="Labelled evaluation JSON file")
    parser.add_argument("--output", type=Path, default=Path("evaluation/results-v0.2.0.json"), help="Destination for JSON metrics")
